...
Claude shallow profiles intentionally do **not** set `CLAUDE_CONFIG_DIR` on
Linux/WSL. Current Claude Code docs describe that variable as the primary
config/auth root: on Linux and Windows, `.credentials.json` moves under that
directory when it is set. caam instead isolates Claude auth by setting
`HOME=<profile>` and managing `<profile>/.claude/.credentials.json`; inherited
`CLAUDE_CONFIG_DIR` is scrubbed so a parent shell cannot repoint a shallow
Claude session at another identity.
...
<shallow-home>/.codex/config.toml   real file, 0600 (sanitized MCP config + file auth store)
...
The Codex `config.toml` is written as a **sanitized managed copy** — it preserves `[mcp_servers.*]` sections so MCP tools still load, enforces `cli_auth_credentials_store = "file"` (so caam can manage `auth.json`), and deliberately does *not* copy your real config's `model`/`log_dir`/`sqlite_home` or model-provider auth aliases. Sharing is the allow-listed CLI transcript state only (`sessions`, `history.jsonl`); full desktop/sidebar history that lives in SQLite is not guaranteed to carry over.
...
**Env isolation:** `shallow-spawn` clears the active harness's repointing vars (`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `CODEX_SQLITE_HOME`, `GEMINI_HOME`), inherited credential/provider-selector vars (`OPENAI_API_KEY`, `CODEX_API_KEY`, `CODEX_ACCESS_TOKEN`, Claude API/token vars, and known Claude cloud-provider auth/selector vars), and caam's own discovery vars (`CAAM_HOME`, `CAAM_SHALLOW_HOMES_DIR`), then sets the active harness's own env (e.g. Codex's `CODEX_HOME`/`CODEX_SQLITE_HOME`). So a shallow profile uses its vaulted subscription identity, not an inherited key. To intentionally use env-key auth, inject it past caam:
...
- Claude shallow auth isolation is **file-based and currently designed/tested for Linux/WSL HOME semantics** (where the subscription credential lives at `~/.claude/.credentials.json`). On **native Windows**, Claude resolves `~/.claude` through `%USERPROFILE%`, so caam must pin the Windows home environment before claiming native-Windows Claude shallow isolation. On **macOS** the credential is in the encrypted **Keychain**, which a per-profile `.credentials.json` + `HOME` redirect does **not** isolate — do not rely on Claude shallow profiles for per-account isolation on macOS. Claude's **secondary** auth (`~/.config/claude-code/auth.json`) is likewise not isolated.
...
- The Claude env scrub list is explicit, not a full cloud-SDK sandbox. If shared Claude settings opt into Bedrock/Vertex/Foundry or another provider and that provider can read credentials from standard SDK config files, instance metadata, keychains, or unlisted env vars, shallow profiles may still share that provider identity. Keep provider-auth settings out of shared `~/.claude/settings.json`, or inject them deliberately after `caam shallow-spawn` when that is the intended identity.
- Codex `config.toml` sanitization is intentionally narrow: shallow create preserves MCP server sections and the file credential-store policy, but drops model/provider settings such as custom `[model_providers.*] env_key` aliases, `log_dir`, and `sqlite_home` so inherited env keys and real-home paths do not authenticate or repoint the shallow session.
...
caam shallow-spawn <name> [--reload-daemon] [--allow-agent-view] -- <cmd> [args...]
...
> **Claude Agent View is disabled by default in shallow sessions (issue #49).** Claude Code's Agent View feature (the `--bg` background-supervisor daemon) runs a **long-lived, cross-session** supervisor process that is **not** bound to the shallow profile's `HOME`. On resume, a shallow `claude` session would reconnect to an already-running supervisor bound to a *different* identity (typically the VM's primary Claude auth), silently bypassing shallow-spawn's per-identity auth isolation and using the wrong account. caam cannot control that daemon's lifecycle, so `caam shallow-spawn <name> -- ...` injects `CLAUDE_CODE_DISABLE_AGENT_VIEW=1` into the child environment by default for every shallow profile, including shells and non-Claude profiles that might launch `claude` later. This keeps Claude foreground and honoring the per-identity `~/.claude/.credentials.json` when used from a Claude shallow profile.
>
